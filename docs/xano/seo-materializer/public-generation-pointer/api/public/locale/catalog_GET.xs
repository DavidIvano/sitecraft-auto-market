query "public/locale/catalog" verb=GET {
  ...
  stack {
    ...
    db.query seo_listing_locale_index {
      where = (($db.seo_listing_locale_index.generation == $active_manifest.generation) && ($db.seo_listing_locale_index.locale_code == $input.lang))
      sort = {
        seo_listing_locale_index.promotion_rank   : "desc"
        seo_listing_locale_index.sort_published_at: "desc"
        seo_listing_locale_index.car_listing_id   : "desc"
      }
    ...
    foreach ($index_page.items) {
      each as $car_id_source {
        array.push $car_ids {
          value = $car_id_source.car_listing_id
        }
      }
    }
            ...
            foreach ($matching_translations) {
              each as $translation_row {
                conditional {
                  if ($translation == null) {
                    var.update $localized_title {
                      value = $translation_row.title|trim
                    }

                    var.update $localized_description {
                      value = $translation_row.description|trim
                    }

                    var.update $localized_seo_title {
                      value = $translation_row.seo_title|first_notnull:""
                    }

                    var.update $localized_seo_description {
                      value = $translation_row.seo_description|first_notnull:""
                    }

                    var.update $localized_image_alt_texts {
                      value = $translation_row.image_alt_texts|first_notnull:[]
                    }
    ...
    precondition (($public_cars|count) == ($index_page.items|count)) {
      error = "SEO catalogue generation is inconsistent"
    }

    db.query seo_sitemap_locale_generations {
      where = ($db.seo_sitemap_locale_generations.is_active)
      sort = {seo_sitemap_locale_generations.locale_code: "asc"}
      return = {type: "list"}
    } as $ready_generation_rows

    var $ready_locales {
      value = []
    }

    foreach ($ready_generation_rows) {
      each as $ready_generation {
        array.push $ready_locales {
          value = $ready_generation.locale_code
        }
      }
    }
  }
  ...
  response = {
    ...
    pagination    : ```
      {
        page       : $index_page.curPage
        limit      : $index_page.perPage
        total      : $index_page.itemsTotal
        total_pages: $index_page.pageTotal
      }
      ```
  ...
  guid = "Y5MWqlecuQSO5_gGA97CZo5DeXg"
}
