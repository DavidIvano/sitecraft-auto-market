query "public/locale/taxonomy/{type}/{slug}/related" verb=GET {
  ...
  stack {
    ...
    db.query seo_taxonomy_locale_stats {
      where = (($db.seo_taxonomy_locale_stats.generation == $source_facet.generation) && ($db.seo_taxonomy_locale_stats.facet_id == $source_facet.id) && ($db.seo_taxonomy_locale_stats.locale_code == $input.lang) && ($db.seo_taxonomy_locale_stats.ready_listing_count > 0))
      return = {type: "single"}
    } as $source_stat
    ...
    function.run "seo_taxonomy/related_groups" {
      input = {
        source_facet_id: $source_facet.id
        generation     : $source_facet.generation
        locale_code    : $input.lang
        limit_per_group: $input.limit_per_group
      }
    } as $groups
  }
