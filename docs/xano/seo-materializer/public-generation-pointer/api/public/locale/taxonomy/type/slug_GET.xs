query "public/locale/taxonomy/{type}/{slug}" verb=GET {
  ...
  stack {
    ...
    db.query seo_taxonomy_facets {
      where = (($db.seo_taxonomy_facets.generation == $active_manifest.generation) && ($db.seo_taxonomy_facets.taxonomy_type == $input.type) && ($db.seo_taxonomy_facets.slug == $input.slug) && (($input.type != "model") || ($db.seo_taxonomy_facets.parent_slug == $input.parent_slug)))
      return = {type: "single"}
    } as $facet
    ...
    db.query seo_taxonomy_locale_stats {
      where = (($db.seo_taxonomy_locale_stats.generation == $facet.generation) && ($db.seo_taxonomy_locale_stats.facet_id == $facet.id) && ($db.seo_taxonomy_locale_stats.locale_code == $input.lang) && ($db.seo_taxonomy_locale_stats.ready_listing_count > 0))
      return = {type: "single"}
    } as $facet_stat
    ...
    db.query seo_taxonomy_listing_edges {
      where = (($db.seo_taxonomy_listing_edges.generation == $facet.generation) && ($db.seo_taxonomy_listing_edges.facet_id == $facet.id) && ($db.seo_taxonomy_listing_edges.locale_code == $input.lang))
      sort = {
        seo_taxonomy_listing_edges.listing_updated_at: "desc"
        seo_taxonomy_listing_edges.car_listing_id    : "desc"
      }
      ...
      each as $edge {
        ...
        foreach ($matching_cars) {
          each as $car {
            ...
            var $translation_meta {
              value = null
            }

            conditional {
              if (($source_locale == $input.lang) && (($car.title|first_notnull:""|trim) != "") && (($car.description|first_notnull:""|trim) != "")) {
                var.update $localized_title {
                  value = $car.title|trim
                }

                var.update $localized_description {
                  value = $car.description|trim
                }
                  ...
                  each as $translation {
                    conditional {
                      if ($translation_meta == null) {
                        var.update $localized_title {
                          value = $translation.title|trim
                        }

                        var.update $localized_description {
                          value = $translation.description|trim
                        }

                        var.update $localized_seo_title {
                          value = $translation.seo_title|first_notnull:""
                        }

                        var.update $localized_seo_description {
                          value = $translation.seo_description|first_notnull:""
                        }

                        var.update $localized_image_alt_texts {
                          value = $translation.image_alt_texts|first_notnull:[]
                        }
    ...
    db.query seo_taxonomy_locale_stats {
      where = (($db.seo_taxonomy_locale_stats.generation == $facet.generation) && ($db.seo_taxonomy_locale_stats.facet_id == $facet.id) && ($db.seo_taxonomy_locale_stats.ready_listing_count > 0))
    ...
    foreach ($ready_locale_stats) {
      each as $ready_locale_stat {
        array.push $ready_locales {
          value = $ready_locale_stat.locale_code
        }
      }
    }
    ...
    conditional {
      if (($facet.taxonomy_type == "model") && (($facet.parent_slug|first_notnull:""|trim) != "")) {
        db.query seo_taxonomy_facets {
          where = (($db.seo_taxonomy_facets.generation == $facet.generation) && ($db.seo_taxonomy_facets.taxonomy_type == "brand") && ($db.seo_taxonomy_facets.slug == $facet.parent_slug))
          return = {type: "single"}
        } as $parent_facet

        conditional {
          if ($parent_facet != null) {
            var.update $parent_label {
              value = $parent_facet.label
            }
          }
        }
      }
    ...
    conditional {
      if (($facet.taxonomy_type == "city") && (($facet.region_slug|first_notnull:""|trim) != "")) {
        db.query seo_taxonomy_facets {
          where = (($db.seo_taxonomy_facets.generation == $facet.generation) && ($db.seo_taxonomy_facets.taxonomy_type == "region") && ($db.seo_taxonomy_facets.slug == $facet.region_slug))
          return = {type: "single"}
        } as $region_facet

        conditional {
          if ($region_facet != null) {
            var.update $region_label {
              value = $region_facet.label
            }
          }
        }
      }
    ...
    function.run "seo_taxonomy/related_groups" {
      input = {
        ...
        generation     : $facet.generation
        locale_code    : $input.lang
        limit_per_group: 8
      }
    } as $related_groups
  }
