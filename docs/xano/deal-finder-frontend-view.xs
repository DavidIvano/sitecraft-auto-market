query "deal-finder/listings/{id}/view" verb=POST {
  ...
  stack {
    ...
    var $next_status { value = "viewed" }
    conditional {
      if ($listing.is_saved == true) {
        var.update $next_status { value = "saved" }
      }
    }
    db.edit deal_finder_listings {
      field_name = "id"
      field_value = $listing.id
      data = {updated_at: "now", is_viewed: true, is_new: false, user_status: $next_status}
    } as $updated
  }
