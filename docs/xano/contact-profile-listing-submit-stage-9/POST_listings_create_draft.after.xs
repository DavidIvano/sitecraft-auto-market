...
query "listings/create-draft" verb=POST {
  ...
  input {
    int draft_id?
    int listing_id?
    text idempotency_key? filters=trim|lower|max:128
  ...
  stack {
    ...
    precondition (($input.idempotency_key == null) || (($input.idempotency_key|strlen) >= 24)) {
    ...
    conditional {
      if (($existing_draft == null) && ($input.idempotency_key != null) && ($input.idempotency_key != "")) {
        db.query car_drafts {
          where = (($db.car_drafts.user_id == $auth.id) && ($db.car_drafts.idempotency_key == $input.idempotency_key))
          return = {type: "single"}
        } as $idempotent_draft

        conditional {
          if ($idempotent_draft != null) {
            precondition (($idempotent_draft.status == "draft") || ($idempotent_draft.status == "ai_draft")) {
              error_type = "inputerror"
              error = "This draft is no longer editable"
            }

            var.update $existing_draft {
              value = $idempotent_draft
            }
          }
        }
      }
    ...
    var $trust_score_value {
      value = null
    }

    conditional {
      if ("/^([0-9]|[1-9][0-9]|100)$/"|regex_matches:$listing_score_raw) {
        var.update $listing_score {
          value = $listing_score_raw|to_int
        }
      }
    }

    conditional {
      if ("/^([0-9]|[1-9][0-9]|100)$/"|regex_matches:$photo_score_raw) {
        var.update $photo_score {
          value = $photo_score_raw|to_int
        }
      }
    }

    conditional {
      if ("/^([0-9]|[1-9][0-9]|100)$/"|regex_matches:$trust_score_raw) {
        var.update $trust_score_value {
          value = $trust_score_raw|to_int
        }
      }
    }
      ...
      if ($existing_draft == null) {
        ...
        db.edit car_drafts {
          ...
          data = {
            ...
            idempotency_key      : $input.idempotency_key|first_notnull:$existing_draft.idempotency_key
            status               : "draft"
  ...
  guid = "-di7aNBJXBZzi2dmdWBak1J1wSA"
}
