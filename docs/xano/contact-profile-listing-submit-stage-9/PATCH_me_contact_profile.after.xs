query "me/contact-profile" verb=PATCH {
  ...
  stack {
    ...
    precondition (($next_phone == null) || ("/^\\+[1-9][0-9]{7,14}$/"|regex_matches:$next_phone)) {
      error_type = "inputerror"
      error = "INVALID_PHONE"
      payload = {code: "PHONE_INVALID", field: "contact_phone", message: "Введите корректный телефон."}
    }

    precondition (($next_show_phone != true) || ($next_phone != null)) {
      error_type = "inputerror"
      error = "PHONE_REQUIRED"
      payload = {code: "PHONE_REQUIRED", field: "contact_phone", message: "Введите телефон или отключите его показ."}
    }

    precondition (($next_email == null) || ("/^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/"|regex_matches:$next_email)) {
      error_type = "inputerror"
      error = "INVALID_EMAIL"
      payload = {code: "EMAIL_INVALID", field: "contact_email", message: "Введите корректный email."}
    }

    precondition (($next_show_email != true) || ($next_email != null)) {
      error_type = "inputerror"
      error = "EMAIL_REQUIRED"
      payload = {code: "EMAIL_REQUIRED", field: "contact_email", message: "Введите email или отключите его показ."}
    }

    precondition (($next_method != "phone") || ($next_show_phone && ($next_phone != null))) {
      error_type = "inputerror"
      error = "PREFERRED_PHONE_NOT_PUBLIC"
      payload = {code: "PREFERRED_CONTACT_UNAVAILABLE", field: "preferred_contact_method", message: "Выбранный телефон недоступен для покупателей."}
    }

    precondition (($next_method != "email") || ($next_show_email && ($next_email != null))) {
      error_type = "inputerror"
      error = "PREFERRED_EMAIL_NOT_PUBLIC"
      payload = {code: "PREFERRED_CONTACT_UNAVAILABLE", field: "preferred_contact_method", message: "Выбранный email недоступен для покупателей."}
    }
