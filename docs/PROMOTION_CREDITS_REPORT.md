...
Production-проверка после исправления:

- listing: `88`, владелец: `15`, статус: `approved`;
- product: `boost_7_days`, idempotency key: `c32749d6-7c0e-4a6c-a9d3-c43eb97b5dd2`;
- HTTP: `200`;
- баланс до: `1000000000`;
- баланс после: `999999995`;
- `boosted_at` и `last_promoted_at`: `1784747876958`;
- `boosted_until`: `1785352676944`;
- ledger: `credit_transactions.id=34`, `amount=-5`, `type=promotion_purchase`, `status=completed`;
- повторный POST с тем же ключом: `DUPLICATE_OPERATION`, без второго списания и без продления срока;
- временный endpoint для выпуска короткоживущего тестового auth token удален сразу после проверки.
...
5. **Не опубликовано:** draft/pending ID; ожидать `LISTING_NOT_PUBLISHED`, без изменений.
6. **Blocked/deleted:** ожидать `LISTING_BLOCKED`, без изменений.
...
Там же сохранены результаты проверки таблиц и endpoint. Обязательных ручных действий в Xano не осталось: таблицы и endpoint опубликованы в `live`, а реальная авторизованная покупка и защита от повторного списания подтверждены production-тестом.
