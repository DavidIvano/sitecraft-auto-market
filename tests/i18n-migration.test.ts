...
test("public cards localize labels and legacy Xano values without changing the payload", () => {
  ...
  assert.match(german, />Kraftstoff<\/dt><dd>Benzin</);
  assert.match(german, />Getriebe<\/dt><dd>Automatik</);
  assert.match(english, />Fuel<\/dt><dd>Petrol</);
  assert.match(english, />Transmission<\/dt><dd>Automatic</);
