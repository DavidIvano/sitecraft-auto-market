...
## Production state — 22 августа 2026

- все 27 целевых локалей обработаны отдельными волнами; исходный русский не требует provider-перевода;
- для каждой из 28 пользовательских локалей strict snapshot подтверждает 11/11 публичных объявлений без fallback;
...
- production Worker работает с `enabled=true`, `dry_run=false`, `scheduled_enabled=true`;
- cron поочерёдно готовит три локали и обрабатывает не более одного задания на локаль; расписание сдвинуто относительно SEO materializer, чтобы не создавать burst в Xano;
